...
router.post('/change-password', authMiddleware, (req, res) => {
  ...
  if (newPassword.length < 12) return res.status(400).json({ error: 'Senha deve ter ao menos 12 caracteres' });
  if (!/[A-Z]/.test(newPassword) || !/[0-9]/.test(newPassword)) {
    return res.status(400).json({ error: 'Senha deve conter letras maiúsculas e números' });
  }
